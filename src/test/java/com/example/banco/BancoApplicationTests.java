 ...
 */
    ...
    public void cadastrarContaNormal() {
        ...
        var userAction = userService.registerUser(user2.getNumUser(),user2.getType(),user2.getSaldo());
        user2.setSaldo(0);
    ...
    void creditarValorContaBonusService() {
        ...
        Assertions.assertThat(userBonus.getPontos()).isEqualTo(user.getPontos()+((int)user.getSaldo()/100));
    }
    @Test
    ...
    void transferirEntreContasNegativoService() {
        ...
        repository.save(user2);
        Assertions.assertThatExceptionOfType(InsufficientFundsException.class).isThrownBy(
                ()-> userService.transfer(user2.getNumUser(), user.getNumUser(), user2.getSaldo()+1001)
        );
    }
